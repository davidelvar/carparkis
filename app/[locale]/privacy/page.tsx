...
export default function PrivacyPage() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        const response = await fetch('/api/settings/pages?page=privacy');
        if (response.ok) {
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 md:p-12">
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-slate-500">
              {locale === 'is' 
                ? 'Persónuverndarstefna hefur ekki verið skilgreind ennþá.'
                : 'Privacy policy has not been defined yet.'}
            </div>
          )}
        </div>
